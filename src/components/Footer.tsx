    ...
    <footer className="bg-brand-950 border-t border-white/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid md:grid-cols-4 gap-8 mb-12">
          <div className="md:col-span-2">
            ...
            <p className="text-white/60 max-w-sm leading-relaxed">
              Curated vintage and pre-owned fashion pieces. Each item tells a unique story of timeless style.
            </p>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {['Home', 'Shop', 'About', 'Contact'].map((link) => (
                <li key={link}>
                  <a href={`#${link.toLowerCase()}`} className="text-white/60 hover:text-brand-400 transition-colors">{link}</a>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="pt-8 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-white/40 text-sm">© {new Date().getFullYear()} EraVault Store. All rights reserved.</p>
