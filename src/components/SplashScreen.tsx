  ...
  return (
    <div className={`fixed inset-0 z-[999] flex items-center justify-center bg-brand-950 transition-opacity duration-500 ${phase === 2 ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        {/* Particle dots — desktop only */}
        <div className="hidden md:contents">
          {Array.from({ length: 12 }).map((_, i) => (
            <div key={i} className="absolute w-1 h-1 bg-brand-400/40 rounded-full animate-float"
              style={{ top: `${Math.random() * 100}%`, left: `${Math.random() * 100}%`, animationDelay: `${Math.random() * 3}s`, animationDuration: `${4 + Math.random() * 3}s` }} />
          ))}
        </div>
      </div>
