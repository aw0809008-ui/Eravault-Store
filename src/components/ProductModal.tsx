...
export default function ProductModal({ product, onClose }: ProductModalProps) {
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowRight' && media.length > 1) goNext();
      if (e.key === 'ArrowLeft' && media.length > 1) goPrev();
    };
  ...
  const goNext = () => {
    if (isAnimating || media.length <= 1) return;
    setIsAnimating(true);
    setDirection('left');
    setTimeout(() => {
  ...
  const slideClass = direction === 'left'
    ...
    : 'animate-[slideEnter_0.5s_cubic-bezier(0.34,1.56,0.64,1)_forwards]';
      ...
      >
        ...
        <div className="relative bg-white rounded-t-[28px] sm:rounded-[28px] overflow-hidden">
          ...
          <button onClick={onClose} className="absolute top-3 right-3 z-30 w-10 h-10 rounded-full bg-black/30 hover:bg-black/60 backdrop-blur-md flex items-center justify-center transition-all group modal-badge-bounce" style={{ animationDelay: '0.6s' }}>
            <svg className="w-5 h-5 text-white transition-transform duration-300 group-hover:rotate-90 group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
          </button>
          ...
          <div className="md:flex max-h-[96vh] sm:max-h-[92vh]">
            ...
            <div className="md:w-[55%] relative bg-gradient-to-br from-gray-950 via-black to-gray-900 flex-shrink-0">
              {/* Main media with 3D slide */}
              <div
                className="relative w-full aspect-[4/5] sm:aspect-square md:aspect-auto md:h-full flex items-center justify-center overflow-hidden cursor-grab active:cursor-grabbing"
              ...
              >
                {media.length > 0 ? (
                  <div key={currentIndex} className={`w-full h-full ${slideClass}`}
                    ...
                    {cur.type === 'image' ? (
                      <img src={cur.url} alt={product.item_name} className="w-full h-full object-contain" />
                    ) : (
                      <video src={cur.url} controls autoPlay playsInline className="w-full h-full object-contain" />
                    )}
                  </div>
                ) : (
                ...
                {/* Arrows — larger, glassier */}
                {media.length > 1 && (
                  <>
                    <button onClick={goPrev} className="absolute left-2 sm:left-3 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/10 hover:bg-white/30 backdrop-blur-xl flex items-center justify-center transition-all duration-300 hover:scale-125 active:scale-90 group border border-white/10">
                      <svg className="w-5 h-5 text-white group-hover:-translate-x-0.5 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15 19l-7-7 7-7" /></svg>
                    </button>
                    <button onClick={goNext} className="absolute right-2 sm:right-3 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/10 hover:bg-white/30 backdrop-blur-xl flex items-center justify-center transition-all duration-300 hover:scale-125 active:scale-90 group border border-white/10">
                      <svg className="w-5 h-5 text-white group-hover:translate-x-0.5 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 5l7 7-7 7" /></svg>
                    </button>
                  </>
                )}
                ...
                {media.length > 1 && (
                  <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-1 text-white/30 text-[10px] sm:hidden modal-btn-slide" style={{ animationDelay: '1s' }}>
              ...
              {/* Thumbnails with glow */}
              {media.length > 1 && (
                <div className="flex gap-1.5 p-2.5 sm:p-3 overflow-x-auto hide-scrollbar bg-black/90 backdrop-blur-md">
                  {media.map((item, i) => (
                    <button key={i} onClick={() => goTo(i)}
                      className={`modal-thumb-enter relative flex-shrink-0 w-14 h-14 sm:w-16 sm:h-16 rounded-xl overflow-hidden transition-all duration-300 ${
                        i === currentIndex
                          ? 'ring-2 ring-brand-400 ring-offset-2 ring-offset-black scale-110 shadow-lg shadow-brand-500/40'
            ...
            <div className="md:w-[45%] overflow-y-auto max-h-[50vh] sm:max-h-none bg-white">
              <div className="p-5 sm:p-7 space-y-5">
                ...
                <div className="flex items-center gap-2 flex-wrap modal-detail-slide">
                  <span className="modal-badge-bounce px-3 py-1 rounded-full bg-brand-100 text-brand-700 text-xs font-bold uppercase tracking-wider" style={{ animationDelay: '0.3s' }}>{product.category}</span>
                  {product.condition && <span className={`modal-badge-bounce px-3 py-1 rounded-full bg-gradient-to-r ${conditionBadge()} text-white text-xs font-bold shadow-md`} style={{ animationDelay: '0.4s' }}>{product.condition}</span>}
                  {product.pieces !== undefined && product.pieces <= 1 && <span className="modal-badge-bounce px-3 py-1 rounded-full bg-gradient-to-r from-red-500 to-pink-500 text-white text-xs font-bold animate-pulse shadow-md" style={{ animationDelay: '0.5s' }}>Last Piece!</span>}
                </div>

                {/* Title */}
                <h2 className="font-display text-2xl sm:text-3xl font-bold text-brand-950 leading-tight modal-detail-slide" style={{ animationDelay: '0.35s' }}>{product.item_name}</h2>
                ...
                <div className="flex flex-wrap gap-2 modal-detail-slide" style={{ animationDelay: '0.5s' }}>
                  {product.size && <div className="px-4 py-2.5 bg-gradient-to-br from-brand-50 to-brand-100/80 rounded-xl border border-brand-200/60"><span className="text-[10px] text-brand-500 uppercase font-bold tracking-wider">Size</span><p className="font-bold text-brand-900 text-sm -mt-0.5">{product.size}</p></div>}
                  {product.pieces !== undefined && <div className="px-4 py-2.5 bg-gradient-to-br from-brand-50 to-brand-100/80 rounded-xl border border-brand-200/60"><span className="text-[10px] text-brand-500 uppercase font-bold tracking-wider">Stock</span><p className="font-bold text-brand-900 text-sm -mt-0.5">{product.pieces} pc{product.pieces !== 1 ? 's' : ''}</p></div>}
                </div>
                ...
                <div className="flex items-center gap-3 p-3.5 bg-gradient-to-r from-blue-50 to-cyan-50 rounded-xl border border-blue-200/60 modal-ship-slide">
                  ...
                  <div><p className="font-bold text-blue-900 text-xs">Worldwide Shipping</p><p className="text-blue-600 text-[11px] leading-snug">Buyer pays shipping. Contact for rates.</p></div>
                </div>
                ...
                <div className="space-y-2.5 pt-1">
                  ...
                  <div className="grid grid-cols-2 gap-2.5">
                    <a href={emailUrl} className="modal-btn-slide flex items-center justify-center gap-2 py-3 bg-brand-50 hover:bg-brand-100 text-brand-800 font-semibold rounded-xl transition-all border border-brand-200 text-sm hover:scale-[1.02] active:scale-[0.98]" style={{ animationDelay: '0.7s' }}>
                    ...
                    <a href="https://instagram.com/Eravault_vintage" target="_blank" rel="noopener noreferrer"
                      className="modal-btn-slide flex items-center justify-center gap-2 py-3 bg-gradient-to-r from-[#833AB4]/10 to-[#E1306C]/10 hover:from-[#833AB4]/20 hover:to-[#E1306C]/20 text-[#E1306C] font-semibold rounded-xl transition-all border border-[#E1306C]/20 text-sm hover:scale-[1.02] active:scale-[0.98]" style={{ animationDelay: '0.75s' }}>
                  ...
                  <p className="modal-btn-slide text-center text-[11px] text-brand-400 pt-1" style={{ animationDelay: '0.85s' }}>📞 +92 323 8226427 &nbsp;•&nbsp; eravaultvintage@gmail.com</p>
                </div>
              </div>
            </div>
          </div>
