...
export default function ProductModal({ product, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 modal-overlay" onClick={onClose}>
      <div className="absolute inset-0 bg-black/85" />

      {/* ── MODAL — FULL SIZE ── */}
      <div
        className="relative w-full sm:max-w-6xl sm:w-[96vw] max-h-[100vh] sm:max-h-[94vh] rounded-t-3xl sm:rounded-3xl overflow-hidden modal-content shadow-2xl"
      ...
      >
        <div className="relative bg-white rounded-t-3xl sm:rounded-3xl overflow-hidden">
          ...
          <div className="md:flex max-h-[100vh] sm:max-h-[94vh]">
            {/* ── MEDIA — 3D CAROUSEL ── */}
            <div className="md:w-[58%] relative bg-gradient-to-br from-brand-50 via-white to-brand-100 flex-shrink-0 select-none">
              <div
                className="relative w-full aspect-square sm:aspect-[4/3] md:aspect-auto md:h-full overflow-hidden"
              ...
              >
                ...
                {/* ARROWS — z-30, above everything */}
                {total > 1 && (
                  <>
                    <button
                      onMouseDown={e => e.stopPropagation()}
                      onClick={e => { e.stopPropagation(); goPrev(); }}
                      className="absolute left-3 top-1/2 -translate-y-1/2 z-30 w-12 h-12 rounded-full bg-white/90 hover:bg-white shadow-xl flex items-center justify-center transition-all hover:scale-110 active:scale-90"
                    >
                      <svg className="w-6 h-6 text-brand-800" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15 19l-7-7 7-7" /></svg>
                    </button>
                    <button
                      onMouseDown={e => e.stopPropagation()}
                      onClick={e => { e.stopPropagation(); goNext(); }}
                      className="absolute right-3 top-1/2 -translate-y-1/2 z-30 w-12 h-12 rounded-full bg-white/90 hover:bg-white shadow-xl flex items-center justify-center transition-all hover:scale-110 active:scale-90"
                    >
                      <svg className="w-6 h-6 text-brand-800" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 5l7 7-7 7" /></svg>
                    </button>
                  </>
                )}
                ...
                <div className="flex justify-center gap-2 py-3 px-3 bg-white border-t border-brand-100">
                  {media.map((item, i) => (
                    <button key={i} onClick={() => setIdx(i)}
                      className={`flex-shrink-0 w-14 h-14 sm:w-16 sm:h-16 rounded-xl overflow-hidden transition-all duration-300 ${i === idx ? 'ring-2 ring-brand-500 ring-offset-2 scale-110 shadow-lg' : 'opacity-40 hover:opacity-70'}`}>
                      {item.type === 'image' ? <img src={item.url} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full bg-brand-100 flex items-center justify-center"><svg className="w-5 h-5 text-red-500" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg></div>}
                    </button>
                  ))}
                </div>
              )}
            ...
            {/* ── DETAILS ── */}
            <div className="md:w-[42%] overflow-y-auto max-h-[50vh] sm:max-h-none bg-white">
